...
class HashTable {
  ...
  int get(String key) {
  int index = hash(key); // Calculate the index using the hash function.
  Node? temp = dataMap?[index]; // Get the first node at the calculated index.

  while (temp != null) {
    if (temp.key == key) {
      // If a node with the same key is found:
      return temp.value; // Return the value associated with the key.
    }
    temp = temp.next;
  }
  return 0; // If the key is not found, return a default value (0 in this case).
}
}
